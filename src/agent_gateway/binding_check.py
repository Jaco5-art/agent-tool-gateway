"""Run local Phase 5 acceptance without any API calls."""
import argparse
import asyncio
...
def main():
    ...
    parser.add_argument('--output',default='results/local-phase5-check.json')
    ...
    report={'phase':5,'mode':'dynamic_binding_with_real_mcp_and_regression','timestamp':datetime.now(timezone.utc).isoformat(),'status':'passed' if result.wasSuccessful() else 'failed','tests_run':result.testsRun,'failures':len(result.failures),'errors':len(result.errors),'skipped':len(result.skipped),'live_model':'not_run_not_required_for_binding_tests','output':stream.getvalue()}
