"""Run local Phase 3 acceptance without any API calls."""
import argparse
import io
import json
import unittest
...
def main():
    ...
    parser.add_argument('--output',default='results/local-phase3-check.json')
    ...
    report={'phase':3,'mode':'capability_matching_with_identity_and_mcp_regression','timestamp':datetime.now(timezone.utc).isoformat(),'status':'passed' if result.wasSuccessful() else 'failed','tests_run':result.testsRun,'failures':len(result.failures),'errors':len(result.errors),'skipped':len(result.skipped),'live_model':'not_run_not_required_for_capability_tests','output':stream.getvalue()}
